动态规划代码，打小抄！！！
1. 使用dp数组 subsetLen[i] 记录 以元素[i] 为 last 元素的最长合规subset长度。如果 i > j，则 subsetLen[i] 可以直接利用 subsetLen[j]的解
    于是 subsetLen[i] = max(subsetLen[i], subsetLen[j] + 1) for (0 <= j < i && nums[j] % nums[i] == 0)
2. 由于需要输出最优解，利用当年floyd输出最优解的思想，使用辅助数组 pre 记录每个元素[i] 对应 subsetLen[i] 时的pre元素
...
public class Solution {
    ...
    public List<Integer> largestDivisibleSubset(int[] nums) {
        ...
        for (int i = 0; i < nums.length; i++) { //其实当 nums.length >= 2时，i 直接从1开始分析也行
            for (int j = 0; j < i; j++) { //为每个元素i：依次取出 [i-1]...[0]这段区间内所有满足 [i]%[j] == 0 的元素[j]进行分析
                if (nums[i] % nums[j] == 0 && subsetLen[j] + 1 > subsetLen[i]) {
        ...
        res.add(nums[maxEndIndex]);
        while (pre[maxEndIndex] != maxEndIndex) { //只有当[i]左侧没有元素使得[i]%[j]==0时，pre[i]才维持初值i
            maxEndIndex = pre[maxEndIndex];
            res.add(nums[maxEndIndex]);
        }
...
TLE递归版代码(结合分支限界法)：对每一层递归的subset，for(i = 本层递归 to nums.length - 1)：if([i] % (subset最后元素) == 0)， 则将元素i添加进入subset，然后递归分析元素[i]

1. 注意剪枝：如果 nums.length - start + 1 <= (当前最优子集长度 - 当前递归层subset长度)，则return

2. 应该向最优子集的head处添加1个dummy元素(取值为1)，避免写如下冗余逻辑 if (subset.size() == 0) {则直接添加元素[i]} 

3. LinkedList的 pop() == removeFirst() == pollFirst()，它移除了first元素；peekLast代表只visit最后元素；removeLast()才是移除最后元素

4. 递归的终止条件不应该写在逻辑的最前方，而应该在 本层递归的 最后，即第i层递归尝试将 nums 剩余的所有元素挑选为 subset 第i个元素 之后
    由于不满足 [i] % subset.getLast() == 0 的元素无法进入下一层递归，所以当最后一个元素不能进入subset时，递归无法进入 start==nums.length 这一层。
    因此当(选择了第i个元素)的第i层递归返回时，应该尝试更新最优解
...
public class Solution {
    ...
    public List<Integer> largestDivisibleSubset(int[] nums) {
        ...
        subset.add(1); //提前添加1个dummy one，于是在第i层递归中选择subset的第i个元素时，就可以避免判断 subset.size()==0 的情况了，因为(any number % 1) == 0
        // for (int i = 0; i < nums.length; i++) {
        // 	subset.removeAll();
        ...
        // 	}
        // }
        helpPartitionSubset(nums, 0, subset);
        res.remove(0);
        return res;
    }

    private void helpPartitionSubset(int[] nums, int start, LinkedList<Integer> subset) {
//        if (start == nums.length && subset.size() > res.size()) {
//            res = subset;
//            return;
//        }
        if (nums.length - start + 1 <= res.size()) { //剪枝
            return;
        }
        int i = start;
        for (; i < nums.length; i++) {
//            if (subset.size() > 0 && subset.getFirst() == 1) {
//                System.out.println(start + " " + i + " " +subset);
//            }
//            if (subset.size() == 0 || nums[i] % subset.getLast() == 0) {
            if (nums[i] % subset.getLast() == 0) {
                subset.add(nums[i]);
                helpPartitionSubset(nums, i + 1, subset);
                subset.removeLast(); //移除最后1个元素，如果只查看最后1个元素则使用 subset.peekLast()
                //subset.pop(); //这个方法 == removeFirst() == pollFirst() ！比较linkedlist相当于双向stack
            } else {
                continue;
            }
        }

        if (i == nums.length && subset.size() > res.size()) { //由于不满足 i%subset.getLast() == 0 的元素无法进入下一层递归，
            // 所以当最后一个元素不能进入subset时，递归无法进入 start==nums.length 这一层。因此当(选择了第i个元素)的第i层递归返回时，应该尝试更新最优解
//            res = subset;
